            ...
            for (int iCol = 0; iCol < 8; ++iCol) {
                if (MainGameBoard[iRow][iCol] != 0) {
                    if (MainGameBoard[iRow][iCol]->GetColor() == PieceColor) {
                        if (MainGameBoard[iRow][iCol]->GetPiece() == "\u265a") {
                            iKingRow = iRow;
                            iKingCol = iCol;
                        }
                    }
...
public:
    ...
    ~ChessBoard() {}
      void Start(int is,int ie) {

    if(!IsGameOver()){
            
            
            GetNextMove(mqGameBoard.MainGameBoard,is,ie);
            if(bValidMove==true){
            AlternateTurn();
              }
          std:: cout << mcPlayerTurn << "'s Move: "<<std::endl;
        }
     else{ 
    mqGameBoard.Print();
     }
...
*/
    void GetNextMove(GamePiece* GameBoard[8][8],int is,int ie) {
        using namespace std;
        bValidMove     = false;
        do {
            system ("clear");
...
/*          cout<<" WP = White pawn &  BP = Black pawn"<<endl;
            ...
            cout<<"Move by selecting row & column to another valid location using row & column"<<endl<<endl<<endl;
             ...
             //   cout << mcPlayerTurn << "'s Move: "<<endl;
       // int iStartMove;
       // cin >> iStartMove;
            int iStartRow = (is / 10) - 1;
            int iStartCol = (is % 10) - 1;
